import sys
with open("d:\\Hivericks\\conversion-system\\voice cloning\\GPT-SoVITS\\GPT_SoVITS\\start_marker.txt", "w") as f: f.write("started\n")
print("S2_TRAIN START", flush=True)
import warnings

# warnings.filterwarnings("ignore")  # Temporarily re-enabled to catch hidden errors
import os

try:
    import utils
    print("About to get hparams...", flush=True)
    hps = utils.get_hparams(stage=2)
    print("Hparams loaded successfully.", flush=True)
    os.environ["CUDA_VISIBLE_DEVICES"] = hps.train.gpu_numbers.replace("-", ",")
    print(f"CUDA_VISIBLE_DEVICES set to: {os.environ['CUDA_VISIBLE_DEVICES']}", flush=True)
    ...
    print("Setting logging levels...", flush=True)
    logging.getLogger("matplotlib").setLevel(logging.INFO)
    logging.getLogger("h5py").setLevel(logging.INFO)
    logging.getLogger("numba").setLevel(logging.INFO)
    from random import randint

    print("Importing internal modules...", flush=True)
    ...
    print(f"IMPORT ERROR: {type(_e).__name__}: {_e}", flush=True)
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
def run(rank, n_gpus, hps):
    print(f"Run function started for rank {rank}")
    global global_step
    if rank == 0:
    ...
    print(f"Initializing process group on {os.name}...")
    if n_gpus > 1:
    ...
    if n_gpus > 1:
        ...
        print("Models wrapped in DDP.")
    else:
        print("Single process mode: skipping DDP wrapping.")
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    if epoch % hps.train.save_every_epoch == 0 and rank == 0:
        ...
        if rank == 0 and hps.train.if_save_every_weights == True:
            ...
            exp_name = os.path.basename(hps.data.exp_dir) if hasattr(hps.data, "exp_dir") else "s2_english"
            logger.info(
                "saving ckpt %s_e%s:%s"
                % (
                    exp_name,
